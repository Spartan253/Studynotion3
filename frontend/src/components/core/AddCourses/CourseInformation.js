      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-end">
        {editCourse && (
          <button
            type="button"
            onClick={() => dispatch(setStep(2))}
            className="w-full sm:w-auto bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded-lg"
          >
            Continue without saving
          </button>
        )}
        ...
        <IconBtn
          text={editCourse ? "Save Changes" : "Next"}
          onClick={handleSubmit(onSubmit)}
          className="w-full sm:w-auto bg-yellow-400 text-black px-6 py-2 rounded-lg"
        />
      </div>
    </form>
  );
